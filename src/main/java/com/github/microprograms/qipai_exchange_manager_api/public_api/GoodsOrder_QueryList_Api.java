...
public class GoodsOrder_QueryList_Api {
    ...
    public static Response execute(Request request) throws Exception {
        ...
        if (!Commons.hasPermission(department, PermissionEnum.orderQuery)) {
        ...
        try (Connection conn = IgniteUtils.getConnection(Consts.jdbc_url)) {
            ...
            ResultSet selectRs = conn.createStatement().executeQuery(new SelectSql(MixOrder.class).where(finalCondition).pager(pagerRequest).sorts(Sort.desc("dtCreate")).build());
    ...
    private static String buildFinalCondition(Req req) {
        ...
        ComplexCondition keyword = Where.and(LikeCondition.build("id", req.getSearchKeyword()), LikeCondition.build("goodsOrder_detail", req.getSearchKeyword()));
        ...
        Condition orderType = Condition.build("orderType=", 1);
