...
public class RoomCard_QueryList_Api {
    ...
    public static Response execute(Request request) throws Exception {
        ...
        try (Connection conn = IgniteUtils.getConnection(Consts.jdbc_url)) {
            ResultSet selectRs = conn.createStatement().executeQuery(new SelectSql(RoomCard.class).pager(pagerRequest).build());
            resp.setData(IgniteUtils.getJavaObjectList(selectRs, RoomCard.class));
            ResultSet selectCountRs = conn.createStatement().executeQuery(new SelectCountSql(RoomCard.class).build());
